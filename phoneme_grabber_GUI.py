...
class USTGeneratorApp:
    ...
    def create_widgets(self):
        self.label1 = ttk.Label(self.frame, text="原始oto.ini文件:")
        ...
        self.browse_table_button = ttk.Button(self.frame, text="浏览", command=self.browse_table)
        self.browse_table_button.grid(row=1, column=2)

        self.label6 = ttk.Label(self.frame, text="输出路径:")
    ...
    def generate_ustobj(self, otoini, notenum, tempo, pause_length_by_beat):
        ...
        if pause_length_by_beat == 'auto':
            ...
            try:
                ...
                print("不支持的暂停长度，请输入一个可用的数字或 'Auto'。")
    ...
    def mono2full_and_round(self, mono_align_dir, full_score_dir, prefix):
        ...
        for path_mono in mono_label_files:
            ...
            if len(mono_label) != len(full_label):
                print(f"错误: {basename(path_mono)} 中的音素数量不匹配")
                continue
        ...
        print('Mono-label 文件已转换为 full-label 文件，并四舍五入.')
    ...
    def generate_ust(self):
        ...
        self.update_output_console('正在将 oto.ini 转换为 lable 文件和 UST 文件并复制 WAV 文件.\n')
